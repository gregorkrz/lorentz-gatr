...
from lgatr.layers.linear import NUM_PIN_LINEAR_BASIS_ELEMENTS
from lgatr.primitives.linear import (
    equi_linear,
    grade_involute,
    grade_project,
    reverse,
...
def test_linear_equivariance(input_batch_dims, coeff_batch_dims):
    ...
    fn_kwargs = dict(
        coeffs=torch.randn(*coeff_batch_dims, NUM_PIN_LINEAR_BASIS_ELEMENTS)
    )
    check_pin_equivariance(
